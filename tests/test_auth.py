...
class TestAuth:
    ...
    def test_admin_endpoint_read_key(self, client: TestClient, read_headers: dict[str, str]) -> None:
        resp = client.get("/api/v1/flags", headers=read_headers)
        assert resp.status_code == 401

    def test_admin_endpoint_admin_key(self, client: TestClient, admin_headers: dict[str, str]) -> None:
    ...
    def test_evaluate_with_admin_key(self, client: TestClient, admin_headers: dict[str, str]) -> None:
        resp = client.post(
            "/api/v1/evaluate",
            json={"flag_key": "test", "env_key": "dev", "user_id": "u1"},
